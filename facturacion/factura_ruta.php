	...
	<body>
		...
		<form id="form_factura" class="was-validated" >
			...
			<div class="container">
				...
				<div class=""  >
					<table class="table table-bordered" style="max-height: 200px; overflow: scroll;">
						<thead>
							<tr>
								<th>CANTIDAD</th>
								<th>DESCRIPCIÓN</th>
						...
						<div class="row my-3">
							<div class="col-12 col-sm-6">
								...
								<div class="input-group">
									<input id="folio_facturacion" class="form-control" value="" placeholder="Folio facturación" type="number" >
									<div class="input-group-append">
										
										<button type="button" id="btn_buscar"  class="btn btn-success  ">
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<button type="submit" id="timbrado_sw"  class="btn btn-primary btn-lg float-right fixed-bottom">
								Generar Factura <i class="fa fa-arrow-right"></i>
							</button>
						</div>
					</div>
					<br>
				...
				<template id="plantilla_concepto">
					<tr class="concepto">
						<td>
							<input  readonly type="number" min="0" step="any"  name="cantidad[]" class="form-control cantidad  text-right" value="1">
						</td>
						<td>
							<textarea  required cols="20"  rows="2"  name="descripcion[]" class="form-control  descripcion"></textarea>
						</td>
						<td>
							<input  readonly type="number" min="0" step="any" name="precio_unitario[]" class="form-control  precio_sin_iva text-right" value="">
						</td>
						<td>
							<input readonly step="any"  name="importe[]" class="form-control importe  text-right" value="">
						</td>
						<td class="d-none">
						...
						<td class="d-none">
							<input  readonly  name="clave_unidad[]" class="clave_unidad  " value="E48">
							<input  readonly  name="nombre_unidades[]" class="unidad" value="Servicio">
							
							<input  readonly type="number" min="0" step="any"  name="clave_producto[]" class="clave_sat" value="78111802">
							
							<input name="tipo_impuesto[0][]" class="tipo_impuesto" value="Traslado">
							<input name="impuesto[0][]" class="impuesto" value="002">
							<input name="base[0][]" class="base" value="">
							<input name="tasa[0][]" class="tasa" value="0.000000">
							<input name="tipo_factor[0][]" class="tipo_factor"  value="Exento">
							<input name="impuesto_importe[][]" step="any"  class="impuesto_importe" value="">
						</td>
					</tr>
				</template>
				...
				<script>
					...
					$("#btn_buscar").click(function(){
						...
						$.ajax({
							"url": "consultas/validar_folio.php",
							"dataType": "JSON",
							"method": "GET",
							"data": {
								"folio_facturacion" : $("#folio_facturacion").val().trim()
							},
							...
							if(respuesta.valido == false){
								...
								$("#folio_facturacion").select()
								return;
							}
							
							plantilla = $($("#plantilla_concepto").html())
							
							plantilla.find(".descripcion").val("Boleto " + respuesta.boleto.id_boletos + ", Servicio Local Aduanas AIFA " )
							plantilla.find(".precio_sin_iva").val(respuesta.boleto.precio)
							plantilla.find(".importe").val(respuesta.boleto.precio)
							plantilla.find(".base").val(respuesta.boleto.precio)
							plantilla.find(".impuesto_importe").val(0)
							
							plantilla.find('input[name^="tipo_impuesto["]').attr('name', 'tipo_impuesto[' + indice + '][]');
							plantilla.find('input[name^="impuesto["]').attr('name', 'impuesto[' + indice + '][]');
							plantilla.find('input[name^="base["]').attr('name', 'base[' + indice + '][]');
							plantilla.find('input[name^="tasa["]').attr('name', 'tasa[' + indice + '][]');
							plantilla.find('input[name^="tipo_factor["]').attr('name', 'tipo_factor[' + indice + '][]');
							plantilla.find('input[name^="impuesto_importe["]').attr('name', 'impuesto_importe[' + indice + '][]');
							
							$("#conceptos").append(plantilla)
							// $("#conceptos").append(plantilla)
							
							$("#folio_facturacion").val("").focus()
					...
					function sumarImportes(){
						...
						$(".concepto").each(function(index, item){
							importe = $(this).find(".precio_sin_iva").val();
