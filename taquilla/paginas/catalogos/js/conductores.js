$(document).ready(function(){
	...
	$('.nuevo').on('click',function(){
		...
		$.each([ "foto_licencia", "foto_certificado", "foto_curso"], function(index, name){
			$("#" + name).closest("div").find("img").attr("src", "")
			$("#"+ name).closest("div").find("a").attr("href", "#").text("Ver foto")
...
function listarRegistros(){
	...
	return $.ajax({
		url: 'consultas/listar_conductores.php',
		method: 'GET',
	...
	$.ajax({
		...
		if(respuesta.estatus == 'success'){
			...
			$.each(respuesta.mensaje[0],function(name , value){
				...
				//Cargar Fotos
				if($.inArray( name, [ "foto_licencia", "foto_certificado", "foto_curso"]) > -1){
