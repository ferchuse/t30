	...
	<body>
		<div class="container">
				...
				<div class="col-xs-2">
					...
					<?php 
						if($_SERVER['SERVER_NAME'] == "localhost"){
							$url_logo = "http://{$_SERVER['SERVER_NAME']}/taxi_demo/taquilla/img/".$factura["url_logo"];
						}
						else{
							$url_logo = "https://{$_SERVER['SERVER_NAME']}/taquilla/img/".$factura["url_logo"];
						}
					...
					<table border="1" class="table table-border table-condensed">
						...
						<?php 
							...
							<tr >
								...
								<td class="col-xs-1 text-right ">$<?php echo number_format($concepto["precio"],2);?></td>
								<td class="col-xs-1 text-right">$<?php echo number_format($concepto["importe"],2);?></td>
							</tr>
			...
			<div class="row ">
				...
				<div class="col-xs-4 ">
					...
					<?php 
						...
						if($factura["factura_global"] == "SI"){
							
							echo "FACTURA GLOBAL: <br>";
							echo "<b>Periodicidad:</b> {$cat_periodicidad[$factura["periodicidad"]]}<br>";
							echo "<b>Mes:</b> {$cat_meses[$factura["mes"]]}<br>";
							echo "<b>Año:</b> {$factura["year"]}<br>";
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody>
						<?php 
							...
							<tr>
								<td class="small"><?php echo $pago["uuid_dr"]?></td>
								<td><?php echo $pago["fecha_pago"]?></td>
								<td><?php echo $pago["forma_pago"]."-" .$cat_forma_pago[$pago["forma_pago"]]?></td>
								<td class="text-right"><?php echo $pago["num_parcialidad"];?></td>
								<td class="text-right">$<?php echo number_format($pago["saldo_anterior"], 2);?></td>
								<td class="text-right">$<?php echo number_format($pago["importe_pagado"], 2);?></td>
								<td class="text-right">$<?php echo number_format($pago["saldo_restante"], 2);?></td>
							</tr>
			...
			<footer <?php if(count($conceptos)  < 7 ) echo " class='footer'";?> >
				<div class="lead text-center <?php echo $factura["timbrado"] == "1" ? "hidden" : "" ?>">
					<?php
						if($factura["orden_pago"] == 1){
							echo "ORDEN DE PAGO";
						}
						else{
							//echo "ORDEN DE PAGO";
				...
				<div class="row <?php echo $factura["timbrado"] == "0" ? "" : "" ?>">
					<div class="col-xs-2">
						
						<?php
							
							echo "<img src='data:image/png;base64,{$factura["qr_code"]}'>";
					...
					<div class="col-xs-9">
						Sello Digital CFDI: <div class="tiny" ><?php echo $factura["representacion_impresa_sello"];?></div>
						Sello SAT : <div class="tiny" > <?php echo $factura["representacion_impresa_selloSAT"];?></div>
						Cadena original del complemento de certificación digital del SAT:
						<div class="tiny" > 
							<?php echo $factura["representacion_impresa_cadena"];?> 
						</div>
					</div>
