$(document).ready(function(){
	 ...
	 $('#id_propietarios').select2({"width": "100%"});
	 $('#form_edicion #id_propietarios').select2({"width": "100%"});
	 $('#form_edicion #id_derroteros').select2({"width": "100%"});
...
function guardarRegistro(event){
	...
	if(confirm("Esta seguro que desea guardar?")){
		...
		$.ajax({
			url: 'concesiones/consultas/guardar_unidades_historial.php',
			dataType: 'JSON',
			method: 'POST',
			data: {
				tabla: 'unidades',
...
function listarRegistros() {
	...
	return $.ajax({
		url: 'concesiones/consultas/listar_unidades.php',
		method: 'GET',
		data: form.serialize()
		}).done(function(respuesta){
...
function mostrarHistorial() {
	...
	$.ajax({
		url: 'concesiones/mostrar_historial.php',
		method: 'GET',
		data: {
			"num_eco": id_registro
		}
